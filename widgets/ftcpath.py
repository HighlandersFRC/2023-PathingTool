...
class FTCPath(Image):
    def __init__(self, sample_func, **kwargs):
        super().__init__(source = "images/ftcfield6.png", **kwargs)
        self.key_points = []
        self.sampled_points = []
        ...
        #robot dimensions in meters, standard 18 by 18 inch limit
        self.robot_length = 0.4572
        self.robot_width = 0.4572
        self.robot_radius = convert.get_robot_radius(self.robot_width, self.robot_length)

        #image of field
        self.field_image = Rectangle(source = "images/ftcfield6.png", pos = self.pos)
    ...
    def draw_path(self):
        ...
        if len(self.key_points) > 1:
            ...
            for i in range(len(self.sampled_points)):
                ...
                px = convert.ftc_meters_to_pixels_x(p[1], self.size)
                py = convert.ftc_meters_to_pixels_y(p[2], self.size)
        ...
        for p in self.key_points:
            pixel_pos = convert.ftc_meters_to_pixels((p.x, p.y), self.size)
            ...
            corner_1 = convert.ftc_meters_to_pixels((self.robot_radius * math.cos(theta_1) + p.x, self.robot_radius * math.sin(theta_1) + p.y), self.size)
            corner_2 = convert.ftc_meters_to_pixels((self.robot_radius * math.cos(theta_2) + p.x, self.robot_radius * math.sin(theta_2) + p.y), self.size)
            corner_3 = convert.ftc_meters_to_pixels((self.robot_radius * math.cos(theta_3) + p.x, self.robot_radius * math.sin(theta_3) + p.y), self.size)
            corner_4 = convert.ftc_meters_to_pixels((self.robot_radius * math.cos(theta_4) + p.x, self.robot_radius * math.sin(theta_4) + p.y), self.size)
            ...
            front = convert.ftc_meters_to_pixels(((self.robot_length / 2.0) * math.cos(p.angle) + p.x, (self.robot_length / 2.0) * math.sin(p.angle) + p.y), self.size)
            ...
            linear_pos = convert.ftc_meters_to_pixels(p.get_vel_marker_pos(), self.size)
        ...
        if len(self.key_points) > 1:
            if self.animation_time <= self.key_points[-1].time and self.animation_time >= 0:
                ...
                anim_pixel_point = convert.ftc_meters_to_pixels((anim_point[0], anim_point[1]), self.size)
                anim_front = convert.ftc_meters_to_pixels(((self.robot_length / 2.0) * math.cos(anim_angle) + anim_point[0], (self.robot_length / 2.0) * math.sin(anim_angle) + anim_point[1]), self.size)
                ...
                anim_corner_1 = convert.ftc_meters_to_pixels((self.robot_radius * math.cos(anim_theta_1) + anim_point[0], self.robot_radius * math.sin(anim_theta_1) + anim_point[1]), self.size)
                anim_corner_2 = convert.ftc_meters_to_pixels((self.robot_radius * math.cos(anim_theta_2) + anim_point[0], self.robot_radius * math.sin(anim_theta_2) + anim_point[1]), self.size)
                anim_corner_3 = convert.ftc_meters_to_pixels((self.robot_radius * math.cos(anim_theta_3) + anim_point[0], self.robot_radius * math.sin(anim_theta_3) + anim_point[1]), self.size)
                anim_corner_4 = convert.ftc_meters_to_pixels((self.robot_radius * math.cos(anim_theta_4) + anim_point[0], self.robot_radius * math.sin(anim_theta_4) + anim_point[1]), self.size)
        ...
        if len(self.recorded_points) > 1:
            if self.recording_animation_time <= self.recorded_points[-1][0] and self.recording_animation_time >= 0:
                ...
                recorded_pixel_point = convert.ftc_meters_to_pixels((recorded_point[1], recorded_point[2]), self.size)
                recorded_front = convert.ftc_meters_to_pixels(((self.robot_length / 2.0) * math.cos(recorded_angle) + recorded_point[1], (self.robot_length / 2.0) * math.sin(recorded_angle) + recorded_point[2]), self.size)
                ...
                recorded_corner_1 = convert.ftc_meters_to_pixels((self.robot_radius * math.cos(recorded_theta_1) + recorded_point[1], self.robot_radius * math.sin(recorded_theta_1) + recorded_point[2]), self.size)
                recorded_corner_2 = convert.ftc_meters_to_pixels((self.robot_radius * math.cos(recorded_theta_2) + recorded_point[1], self.robot_radius * math.sin(recorded_theta_2) + recorded_point[2]), self.size)
                recorded_corner_3 = convert.ftc_meters_to_pixels((self.robot_radius * math.cos(recorded_theta_3) + recorded_point[1], self.robot_radius * math.sin(recorded_theta_3) + recorded_point[2]), self.size)
                recorded_corner_4 = convert.ftc_meters_to_pixels((self.robot_radius * math.cos(recorded_theta_4) + recorded_point[1], self.robot_radius * math.sin(recorded_theta_4) + recorded_point[2]), self.size)
        ...
        #draw selected point
        if self.selected_point != None:
            pixel_pos = convert.ftc_meters_to_pixels((self.selected_point.x, self.selected_point.y), self.size)
        ...
        if len(self.recorded_points) > 0:
            ...
            xy_list[::2] = [convert.ftc_meters_to_pixels_x(p[1], self.size) for p in self.recorded_points]
            xy_list[1::2] = [convert.ftc_meters_to_pixels_y(p[2], self.size) for p in self.recorded_points]
        ...
        for c in self.commands:
            ...
            if c["trigger_type"] == "time":
                ...
                px, py = convert.ftc_meters_to_pixels(p, self.size)
            elif c["trigger_type"] == "position":
                px = convert.ftc_meters_to_pixels_x(c["trigger"][0], self.size)
                py = convert.ftc_meters_to_pixels_y(c["trigger"][1], self.size)
    ...
    def get_selected_point(self, px, py):
        for p in self.key_points:
            pixel_pos = convert.ftc_meters_to_pixels((p.x, p.y), self.size)
            # vel_marker_pos = convert.ftc_meters_to_pixels(p.get_vel_marker_pos(), self.size)
    ...
    def update_info(self):
        pos = convert.get_ftc_cursor_field_pos_meters(self.size)
        pixel_pos = convert.get_cursor_screen_pos_pixels()
        if self.selected_point != None:
            dist = convert.get_ftc_cursor_dist_meters([self.selected_point.x, self.selected_point.y], self.size)
        ...
        if texture == None:
            return
        self.info_rect = Rectangle(texture = texture, size = list(texture.size), pos = (35, 567))
